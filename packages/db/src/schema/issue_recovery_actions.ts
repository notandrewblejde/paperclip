...
export const issueRecoveryActions = pgTable(
  "issue_recovery_actions",
  {
    ...
    wakePolicy: jsonb("wake_policy"),
    monitorPolicy: jsonb("monitor_policy"),
  ...
  (table) => ({
    ...
    activeSourceUq: uniqueIndex("issue_recovery_actions_active_source_uq")
      .on(table.companyId, table.sourceIssueId)
      .where(sql`${table.status} in ('active', 'escalated')`),
    activeFingerprintUq: uniqueIndex("issue_recovery_actions_active_fingerprint_uq")
